...
export default function AdminPromoCodesScreen() {
  ...
  const { data: codes = [], isLoading, refetch, isRefetching } = useAdminPromoCodes(
    filterTab === 'all' ? undefined : { isActive: filterTab === 'active' }
  );
  ...
  const handleSubmit = () => {
    ...
    createMutation.mutate(
      ...
      {
        onSuccess: () => {
          CustomInAppToast.show({ type: 'success', title: 'Created', message: 'Platform-wide promo code created.' });
    ...
    <View style={styles.container}>
      ...
      <View style={styles.tabContainer}>
        {FILTERS.map((f) => (
          <TouchableOpacity
            key={f.key}
            style={[styles.tab, filterTab === f.key && styles.tabActive]}
            onPress={() => setFilterTab(f.key)}
          >
            <Text style={[styles.tabText, filterTab === f.key && styles.tabTextActive]}>{f.label}</Text>
          </TouchableOpacity>
        ))}
      </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptyDesc}>No codes match this filter yet.</Text>
            </View>
          }
          renderItem={({ item }) => (
...
const getStyles = (c: ThemeColors) => StyleSheet.create({
  ...
  tabContainer: {
    flexDirection: 'row', backgroundColor: c.surface, paddingHorizontal: 16,
    borderBottomWidth: 1, borderBottomColor: c.border,
  },
  tab: { flex: 1, alignItems: 'center', paddingVertical: 10, borderBottomWidth: 2, borderBottomColor: 'transparent' },
  tabActive: { borderBottomColor: c.primary },
  tabText: { fontSize: rf(13), fontFamily: 'Montserrat-SemiBold', color: c.textMuted },
  ...
  storeText: { fontSize: rf(11), fontFamily: 'Montserrat-SemiBold', color: c.textSecondary, marginBottom: 4 },
