...
export default function CategoryScreen() {
  ...
  // 🔍 Filter Logic
  // If search is empty, show ONLY Categories.
  // If searching, show Categories AND Products that match.
  ...
  const handlePress = (item: any) => {
    if (item.type === 'category') {
        console.log(`Open Category: ${item.title}`);
        ...
        router.push({
            ...
            params: {
                ...
                title: item.title,
                price: item.price,
                image: item.image, // Pass the asset ID
...
const styles = StyleSheet.create({
  ...
  // 2. Product Card Style (Distinct look for search results)
  productCard: {
    flex: 1,
    margin: 8,
    height: 180,
