...
describe('Reviews Service Unit Tests', () => {
  ...
  describe('getStoreReviews', () => {
    test('test_getStoreReviews_validId_returnsReviewsAnd200', async () => {
      ...
      (api.get as jest.Mock).mockResolvedValueOnce({ data: { success: true, reviews: mockReviews } });

      // Act
      const result = await getStoreReviews('store-123', { limit: 10 });

      // Assert
      expect(api.get).toHaveBeenCalledWith('/reviews/store/store-123', { params: { limit: 10 } });
  ...
  describe('getProductReviews', () => {
    test('test_getProductReviews_validId_returnsReviewsAnd200', async () => {
      ...
      (api.get as jest.Mock).mockResolvedValueOnce({ data: { success: true, data: mockReviews } });

      // Act
      const result = await getProductReviews('prod-123');

      // Assert
      expect(api.get).toHaveBeenCalledWith('/reviews/product/prod-123', { params: {} });
      expect(result.reviews).toEqual(mockReviews);
    });

    test('test_getProductReviews_apiFails_throwsError', async () => {
      // Arrange
      (api.get as jest.Mock).mockRejectedValueOnce(new Error('Product not found'));
  ...
  describe('createProductReview', () => {
    test('test_createProductReview_validData_returnsCreatedReview', async () => {
      ...
      (api.post as jest.Mock).mockResolvedValueOnce({ data: mockResData });
      ...
      // Assert
      expect(api.post).toHaveBeenCalledWith('/reviews/product', reviewInput);
      expect(result).toEqual(mockResData);
    });

    test('test_createProductReview_apiFails_throwsError', async () => {
      // Arrange
      (api.post as jest.Mock).mockRejectedValueOnce(new Error('Prior purchase required'));
  ...
  describe('createStoreReview', () => {
    test('test_createStoreReview_validData_returnsCreatedReview', async () => {
      ...
      // Assert
      expect(api.post).toHaveBeenCalledWith('/reviews/store', reviewInput);
      expect(result).toEqual(mockResData);
    });

    test('test_createStoreReview_apiFails_throwsError', async () => {
      // Arrange
      (api.post as jest.Mock).mockRejectedValueOnce(new Error('Invalid order'));
  ...
  describe('createDriverReview', () => {
    test('test_createDriverReview_validData_returnsCreatedReview', async () => {
      ...
      // Assert
      expect(api.post).toHaveBeenCalledWith('/reviews/driver', reviewInput);
      expect(result).toEqual(mockResData);
    });

    test('test_createDriverReview_apiFails_throwsError', async () => {
      // Arrange
      (api.post as jest.Mock).mockRejectedValueOnce(new Error('Driver not assigned'));
  ...
  describe('getReviewableProducts', () => {
    test('test_getReviewableProducts_validCall_returnsProducts', async () => {
      ...
      (api.get as jest.Mock).mockResolvedValueOnce({ data: { success: true, products: mockProducts } });

      // Act
      const result = await getReviewableProducts();

      // Assert
      expect(api.get).toHaveBeenCalledWith('/reviews/reviewable-products');
  ...
  describe('likeReview', () => {
    test('test_likeReview_validId_returnsSuccessData', async () => {
      ...
      // Assert
      expect(api.post).toHaveBeenCalledWith('/reviews/rev-123/like');
      expect(result).toEqual(mockResData);
    });
  });
  ...
  describe('getReviewComments', () => {
    test('test_getReviewComments_validId_returnsComments', async () => {
      ...
      (api.get as jest.Mock).mockResolvedValueOnce({ data: { success: true, comments: mockComments } });

      // Act
      const result = await getReviewComments('rev-123');

      // Assert
      expect(api.get).toHaveBeenCalledWith('/reviews/rev-123/comments');
  ...
  describe('createReviewComment', () => {
    test('test_createReviewComment_validInput_returnsCreatedComment', async () => {
      ...
      // Assert
      expect(api.post).toHaveBeenCalledWith('/reviews/rev-123/comments', { text: 'Agree!' });
      expect(result).toEqual(mockResData);
    });
  });
});
