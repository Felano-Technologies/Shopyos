...
function _userHash(userId) {
  return [...userId].reduce((h, c) => (h * 31 + c.charCodeAt(0)) & 0xffffffff, 0);
}
...
function pickVariant(variants, userId, salt) {
  const h = [...(userId + salt)].reduce((acc, c) => (acc * 31 + c.charCodeAt(0)) & 0xffffffff, 0);
    ...
    try {
      ...
      if (customersRole && customersRole.length > 0) {
        ...
        for (let i = 0; i < allIds.length; i += PAGE_SIZE) {
          ...
          await runInBatches(customers, async (c) => {
            ...
            const pushScreen =
              contentType === 'product_spotlight' && spotlightCtx.productId ? 'product' :
              contentType === 'store_spotlight'   && spotlightCtx.storeId   ? 'store'   : 'notifications';
            const pushExtra =
              contentType === 'product_spotlight' && spotlightCtx.productId ? { productId: spotlightCtx.productId } :
              contentType === 'store_spotlight'   && spotlightCtx.storeId   ? { storeId: spotlightCtx.storeId }     : {};
            return notificationService.sendNotification({
              ...
              push: sendPush ? {
                data: { screen: pushScreen, type: 'daily_engagement', ...pushExtra }
              } : null
