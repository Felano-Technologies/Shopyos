...
class QueryBuilder {
  ...
  async execute() {
    const db = getPool();
    try {
      if (this.operation === 'insert' || this.operation === 'upsert') return await this._executeInsertUpsert(db);
      if (this.operation === 'update') return await this._executeUpdate(db);
      if (this.operation === 'delete') return await this._executeDelete(db);
      return await this._executeSelect(db);
    } catch (error) {
  ...
  async _executeInsertUpsert(db) {
    ...
    if (this._ignoreConflict) {
      ...
      return { data: serializeDates(inserted), error: null, rowCount, conflictIgnored };
    }
  ...
  async _executeSelect(db) {
    ...
    if (finalSelect.includes('(') || finalSelect.includes(':')) finalSelect = '*';

    const sql = `SELECT ${finalSelect} FROM ${this.tableName}${clause}${orderSql}${limitSql}${offsetSql}`;
    const result = await db.query(sql, values);

    if (result.rows.length > 0) await this._applyJoinShims(result, db);

    result.rows = serializeDates(result.rows);

    if (this.singleMode) {
      if (result.rows.length === 0) return { data: null, error: toPgError(new Error('No rows'), true) };
      return { data: result.rows[0], error: null };
    }

    if (this.maybeSingleMode) return { data: result.rows[0] || null, error: null };
  ...
  async _applyJoinShims(result, db) {
    if (this.tableName === 'user_roles') { await this._shimUserRoles(result, db); return; }
    if (this.tableName === 'users') { await this._shimUsers(result, db); return; }
    if (this.tableName === 'carts') { await this._shimCarts(result, db); return; }
    if (this.tableName === 'orders') { await this._shimOrders(result, db); return; }
    if (this.tableName === 'products') { await this._shimProducts(result, db); return; }
    if (this.tableName === 'favorites') { await this._shimFavorites(result, db); return; }
    if (this.tableName === 'messages') { await this._shimMessages(result, db); return; }
    if (this.tableName === 'conversations') { await this._shimConversations(result, db); return; }
    if (this.tableName === 'stores') { await this._shimStores(result, db); return; }
    if (this.tableName === 'deliveries') { await this._shimDeliveries(result, db); }
  }

  async _shimUserRoles(result, db) {
    ...
    result.rows = result.rows.map(row => ({
      ...
      roles: roleMap[row.role_id] || null
    }));
  }

  async _shimUsers(result, db) {
  ...
  async _shimOrders(result, db) {
    ...
    if (storeIds.length > 0) {
      const { rows: stores } = await db.query(`SELECT * FROM stores WHERE id = ANY($1)`, [storeIds]);
  ...
  async _shimFavorites(result, db) {
  ...
  async _shimConversations(result, db) {
  ...
  async _fetchDeliveryOrders(orderIds, db) {
    const { rows: orders } = await db.query(`SELECT * FROM orders WHERE id = ANY($1)`, [orderIds]);
    ...
    if (buyerIds.length > 0) {
      ...
      buyers.forEach(b => {
        buyerMap[b.id] = {
          id: b.id,
          email: b.email,
          user_profiles: { full_name: b.full_name, phone: b.phone, avatar_url: b.avatar_url }
        };
      });
    }

    const { rows: orderItems } = await db.query(`SELECT * FROM order_items WHERE order_id = ANY($1)`, [orderIds]);
  ...
  async _shimDeliveries(result, db) {
    ...
    let orderMap = {};
    if (orderIds.length > 0) orderMap = await this._fetchDeliveryOrders(orderIds, db);
    ...
    if (driverIds.length > 0) {
      ...
      drivers.forEach(drv => {
        driverMap[drv.id] = {
          id: drv.id,
          email: drv.email,
          user_profiles: { full_name: drv.full_name, phone: drv.phone, avatar_url: drv.avatar_url }
        };
      });
    }
    ...
    if (deliveryIds.length > 0) {
      ...
      locationUpdates.forEach(update => {
        if (!locationUpdatesMap[update.delivery_id]) locationUpdatesMap[update.delivery_id] = [];
