    ...
    <>
      ...
      <div className="flex flex-col gap-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-body">Logistics Hubs & Transit</h1>
            <p className="text-sm text-secondary mt-1">Manage regional parcel hubs and inter-regional transit routing.</p>
          </div>
          <button
          ...
          {[
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-semibold border transition-colors ${
                activeTab === tab.id ? 'bg-navy text-white border-navy' : 'bg-card text-secondary border-border hover:border-navy/30'
              }`}
            >
              {tab.label}
            </button>
          ...
          {loading ? (
            ...
            hubs.length === 0 ? (
              <div className="text-center p-12 text-secondary">
                <FiMapPin className="w-10 h-10 mx-auto mb-3 text-subtle" />
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-surface-muted/50 border-b border-border">
                      <th className="px-6 py-4 text-xs font-semibold text-secondary uppercase tracking-wider">Hub</th>
                      <th className="px-6 py-4 text-xs font-semibold text-secondary uppercase tracking-wider">Region</th>
                    ...
                    {hubs.map((h) => (
                      <tr key={h.id} className="hover:bg-surface-muted/50 transition-colors">
                        <td className="px-6 py-4">
                          <div className="font-medium text-body">{h.hub_name}</div>
                          <div className="text-sm text-secondary">{h.partner_name}</div>
                        </td>
                        <td className="px-6 py-4 text-sm text-body">
                          <div className="flex items-center gap-1.5"><FiMap className="w-3.5 h-3.5 text-subtle" /> {h.region_name || `Region ${h.region_id}`}</div>
                          {h.address && <div className="text-xs text-subtle mt-0.5 max-w-xs truncate">{h.address}</div>}
                        </td>
                        <td className="px-6 py-4 text-sm text-body">
                          {h.phone ? <div className="flex items-center gap-1.5"><FiPhone className="w-3.5 h-3.5 text-subtle" /> {h.phone}</div> : <span className="text-subtle">—</span>}
                        </td>
                        <td className="px-6 py-4">
                          <button
                            onClick={() => handleToggleHub(h)}
                            className={`px-2.5 py-1 rounded-md text-xs font-semibold transition-colors ${
                              h.is_active ? 'bg-green-50 text-green-700 hover:bg-green-100' : 'bg-surface-muted text-secondary hover:bg-border-strong'
                            }`}
                          >
            ...
            <div className="text-center p-12 text-secondary">
              <FiMap className="w-10 h-10 mx-auto mb-3 text-subtle" />
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-surface-muted/50 border-b border-border">
                    <th className="px-6 py-4 text-xs font-semibold text-secondary uppercase tracking-wider">Route</th>
                    <th className="px-6 py-4 text-xs font-semibold text-secondary uppercase tracking-wider">Transit Time</th>
                    ...
                    <tr key={r.id} className="hover:bg-surface-muted/50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2 text-sm">
                          <span className="font-semibold text-body">{r.origin_region}</span>
                          <FiMapPin className="text-subtle w-3.5 h-3.5" />
                          <span className="font-semibold text-body">{r.dest_region}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-sm text-body">{r.transit_days_min}–{r.transit_days_max} days</td>
                      <td className="px-6 py-4 text-sm font-semibold text-body">₵{Number(r.transit_fee).toFixed(2)}</td>
                      <td className="px-6 py-4 text-right">
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-card rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="px-6 py-4 border-b border-border">
              <h2 className="text-lg font-bold text-body">{hubForm.id ? 'Edit Hub' : 'New Hub'}</h2>
            </div>
            <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
              ...
              {!hubForm.id && (
                <div>
                  <label className="block text-sm font-semibold text-body mb-1">Region</label>
                  <select
              ...
              <div>
                <label className="block text-sm font-semibold text-body mb-1">Hub Name</label>
                <input type="text" value={hubForm.hubName} onChange={(e) => setHubForm({ ...hubForm, hubName: e.target.value })} placeholder="e.g. Accra Central Hub" className="w-full px-4 py-2.5 rounded-xl border border-border text-sm focus:outline-none focus:ring-2 focus:ring-navy/10 focus:border-navy" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-body mb-1">Partner / Distributor Name</label>
                <input type="text" value={hubForm.partnerName} onChange={(e) => setHubForm({ ...hubForm, partnerName: e.target.value })} placeholder="e.g. Express Ghana Ltd" className="w-full px-4 py-2.5 rounded-xl border border-border text-sm focus:outline-none focus:ring-2 focus:ring-navy/10 focus:border-navy" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-body mb-1">Address</label>
                <textarea value={hubForm.address} onChange={(e) => setHubForm({ ...hubForm, address: e.target.value })} rows={2} className="w-full px-4 py-2.5 rounded-xl border border-border text-sm focus:outline-none focus:ring-2 focus:ring-navy/10 focus:border-navy" />
              </div>
              <div>
            ...
            <div className="px-6 py-4 border-t border-border flex items-center justify-end gap-3 bg-surface-muted/50">
              <button onClick={() => setIsHubModalOpen(false)} className="px-5 py-2.5 rounded-xl text-sm font-semibold text-secondary hover:bg-surface-muted transition-colors">Cancel</button>
      ...
      {isRouteModalOpen && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-card rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="px-6 py-4 border-b border-border">
              <h2 className="text-lg font-bold text-body">Configure Transit Route</h2>
            </div>
            <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
              {formError && (
                <div className="bg-red-50 text-red-600 p-3 rounded-lg text-sm font-medium border border-red-100">{formError}</div>
              ...
              <div>
                <label className="block text-sm font-semibold text-body mb-1">Origin Region</label>
                <select
              ...
              <div>
                <label className="block text-sm font-semibold text-body mb-1">Destination Region</label>
                <select
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold text-body mb-1">Min Days</label>
                  <input type="number" min={1} value={routeForm.transitDaysMin} onChange={(e) => setRouteForm({ ...routeForm, transitDaysMin: e.target.value })} className="w-full px-4 py-2.5 rounded-xl border border-border text-sm focus:outline-none focus:ring-2 focus:ring-navy/10 focus:border-navy" />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-body mb-1">Max Days</label>
                  <input type="number" min={1} value={routeForm.transitDaysMax} onChange={(e) => setRouteForm({ ...routeForm, transitDaysMax: e.target.value })} className="w-full px-4 py-2.5 rounded-xl border border-border text-sm focus:outline-none focus:ring-2 focus:ring-navy/10 focus:border-navy" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-semibold text-body mb-1">Transit Fee (₵)</label>
                <input type="number" min={0} step="0.01" value={routeForm.transitFee} onChange={(e) => setRouteForm({ ...routeForm, transitFee: e.target.value })} className="w-full px-4 py-2.5 rounded-xl border border-border text-sm focus:outline-none focus:ring-2 focus:ring-navy/10 focus:border-navy" />
              </div>
            </div>
            <div className="px-6 py-4 border-t border-border flex items-center justify-end gap-3 bg-surface-muted/50">
              <button onClick={() => setIsRouteModalOpen(false)} className="px-5 py-2.5 rounded-xl text-sm font-semibold text-secondary hover:bg-surface-muted transition-colors">Cancel</button>
