  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Fee & Commission Settings</h1>
          <p className="text-sm text-gray-500 mt-1">Configure platform-wide fees and parameters.</p>
        </div>
              ...
              {TABS.map((tab) => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 text-sm font-semibold transition-colors border-l-2 ${
                    activeTab === tab.key ? 'border-navy bg-gray-50 text-navy' : 'border-transparent text-gray-600 hover:bg-gray-50 hover:text-navy'
                  }`}
                >
              ...
              {loading ? (
                ...
                <div className="flex flex-col">
                  {visibleConfigs.map((config) => {
                    ...
                    return (
                      <div key={config.id} className="py-5 border-b border-gray-100 last:border-0 last:pb-0 first:pt-0">
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            ...
                            <button onClick={() => openAudit(config)} className="p-2 text-gray-400 hover:text-navy hover:bg-gray-100 rounded-lg transition-colors" title="Change history">
                              <FiClock className="w-4 h-4" />
                            </button>
                            <button onClick={() => openEdit(config)} className="p-2 text-gray-400 hover:text-navy hover:bg-gray-100 rounded-lg transition-colors" title="Edit value">
                      ...
                      <table className="w-full text-left border-collapse">
                        <thead>
                          <tr className="bg-gray-50/50 border-b border-gray-100">
                            ...
                            <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Products Used</th>
                            ...
                            return (
                              <tr key={s.id} className="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td className="px-6 py-4">
                                  <span className={`px-2.5 py-1 rounded-md text-xs font-semibold capitalize ${s.listing_tier === 'paid' ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
                                    {s.listing_tier}
                                  </span>
                                </td>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
              <h2 className="text-lg font-bold text-gray-900">Update Configuration</h2>
            ...
            <div className="p-6 space-y-4">
              <div>
                <p className="font-semibold text-gray-900">{editingConfig.label}</p>
                {editingConfig.description && <p className="text-sm text-gray-500 mt-1">{editingConfig.description}</p>}
              </div>
              {editError && (
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-end gap-3 bg-gray-50/50">
              <button onClick={() => setEditingConfig(null)} disabled={saving} className="px-5 py-2.5 rounded-xl text-sm font-semibold text-gray-600 hover:bg-gray-100 transition-colors">Cancel</button>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500">{auditConfig.label}</p>
              </div>
              <button onClick={() => setAuditConfig(null)} className="text-gray-400 hover:text-gray-600">
                <FiX className="w-5 h-5" />
              </button>
            </div>
                ...
                <div className="flex flex-col divide-y divide-gray-100">
                  {auditLogs.map((log) => (
                    <div key={log.id} className="py-3 first:pt-0">
                      ...
                      {log.reason && <p className="text-xs text-gray-500 italic mt-1 bg-gray-50 rounded-md px-2 py-1">"{log.reason}"</p>}
                    </div>
                  ))}
                </div>
              )}
