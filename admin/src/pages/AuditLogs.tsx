  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-6">
        <div>
          <h1 className="text-2xl font-bold text-body">System Audit Logs</h1>
          <p className="text-sm text-secondary mt-1">Track administrative and system-wide activity.</p>
        </div>
                ...
                {logs.map((log) => {
                  ...
                  return (
                    <div key={log.id} className="relative pl-8">
                      <div className={`absolute -left-[1.35rem] top-1 w-10 h-10 rounded-full bg-card border flex items-center justify-center shadow-sm ${failed ? 'border-red-200' : 'border-border'}`}>
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <div className="bg-surface-muted p-4 rounded-xl border border-border mt-2">
                          <div className="flex items-start justify-between gap-3">
                            <p className="text-body font-medium text-sm capitalize">{formatAction(log.action)}</p>
                          ...
                          {log.entity_type && (
                            <div className="flex items-center gap-2 mt-2 text-xs text-secondary">
                              ...
                              {log.entity_id && <span className="font-mono text-subtle">{log.entity_id.slice(0, 8)}</span>}
                            </div>
                          )}

                          <div className="flex items-center gap-2 mt-3 text-sm text-secondary border-t border-border pt-3">
                            <FiUser className="w-3.5 h-3.5" />
                            <span>
                              {log.actor?.full_name || log.actor?.email || 'System'}
                              {log.actor?.role && <span className="text-subtle"> · {log.actor.role}</span>}
                            </span>
                          </div>
                        </div>
                      </div>
