    ...
    try {
      ...
      if (res) {
        setSettings({
          ...
          auto_approve_sellers: res.auto_approve_sellers || false,
        });
      }
    } catch (err) {
      console.error(err);
    ...
    <>
      ...
      <div className="flex flex-col gap-6 max-w-4xl">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Configure global platform rules, notifications, and security.</p>
        </div>

        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          {/* Tabs */}
          <div className="flex border-b border-gray-100">
            <button className="flex items-center gap-2 px-6 py-4 text-sm font-semibold text-navy border-b-2 border-navy">
              <FiSettings /> General
            </button>
            <button className="flex items-center gap-2 px-6 py-4 text-sm font-semibold text-gray-500 hover:text-gray-700 transition-colors">
            ...
            {loading ? (
              ...
              <>
                <div className="flex flex-col gap-6">
                  <h3 className="text-lg font-bold text-gray-900">Platform Toggles</h3>
                ...
                <div className="flex justify-end mt-4">
                  <button 
                    onClick={handleSave}
                    disabled={submitting}
                    className="flex items-center gap-2 bg-navy text-white px-6 py-3 rounded-xl text-sm font-semibold hover:bg-navy/90 transition-colors shadow-sm disabled:opacity-50"
                  >
                    <FiSave /> Save Changes
                  </button>
                </div>
