...
class SocketService {
  ...
  private getSocketURL(): string {
    ...
    if (isDev) {
      // In dev mode, route sockets through local Vite proxy
      return window.location.origin;
    }
    const socketURL = import.meta.env.EXPO_PUBLIC_SOCKET_URL?.trim();
    if (!socketURL) {
        ...
        try {
          ...
          this.socket.on('connect_error', async (error: any) => {
            ...
            if (message.includes('jwt expired') || message.includes('Authentication failed')) {
              ...
              try {
                // Trigger token refresh interceptor via lightweight me request
                await api.get('/auth/me'); 
                
                // Fetch refreshed token and reconnect
