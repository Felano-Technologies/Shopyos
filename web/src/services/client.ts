...
const getBaseURL = () => {
  ...
  if (isDev) {
    // Vite Dev Proxy handles this relative call to avoid CORS preflight failures
    return '';
  }
  const apiUrl = import.meta.env.EXPO_PUBLIC_API_URL?.trim();
...
async function handle429(error: any, originalRequest: any): Promise<any> {
  ...
  // Dispatch custom event to show notification in React UI
  window.dispatchEvent(new CustomEvent('app-toast', {
    detail: { type: 'error', title: 'Too many requests', message: 'Please slow down and try again.' }
  }));
  throw error;
}

async function handleTokenExpired(_error: any, originalRequest: any): Promise<any> {
  if (isRefreshing) {
